...
export async function GET(
  ...
  try {
    ...
    return handleApiError(error, "[GET /api/institutions/:id/knowledge]");
  }
}
...
export async function POST(
  ...
  try {
    ...
    void logActivity(session.user.id, "CREATE", "KnowledgeBase", article.id, { route: "institutions/[id]/knowledge" });

    return NextResponse.json({ article }, { status: 201 });
  } catch (error) {
    return handleApiError(error, "[POST /api/institutions/:id/knowledge]");
  }
}
